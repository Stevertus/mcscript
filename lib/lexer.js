...
function TokenStream(input) {
	...
	function read_next() {
		...
		if (ch == "/" && input.peekNext(1) == "*") {
			skip_line_comment();
			// TODO Is this line really useless ???
			// It works correctly only after I comment out the line below.
			// read_next();
			return read_next();
		}
		...
		// This means current char is 'r in "run:"
		if (
			ch == "r" &&
			input.peekNext(1) == "u" && // I know it does works
			input.peekNext(2) == "n" && // But it looks so weird
			input.peekNext(3) == ":"
		) {
			return read_command();
		}
