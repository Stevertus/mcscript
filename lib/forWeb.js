  ...
  InputStream = function(input) {
    ...
    function getCol() {
      ...
      while(" \r\n".indexOf(input.charAt(pos2)) >= 0 && num >= 1){
        console.log(JSON.stringify(input.charAt(pos2)))
        pos2--
        num--
      }
      console.log(num)
  ...
  parse = function(input) {
    ...
    function parse_if() {
      ...
      input.next()
      let not = false
      var cond = input.next()//parse_expression();
      if(cond.value == "!"){
        not = true
        cond = input.next()
      }
  ...
  generate = function(exp) {
    ...
    function js(exp) {
      switch (exp.type) {
        ...
        case "if"     : return make_indent(exp,"if");
        case "unless" : return make_indent(exp,"unless");
    ...
    function make_indent(exp,name) {
      ...
      if(exp.then.type == "prog"){
        for(comm of exp.then.prog){
          ...
          let subTree = js(comm).split("\n")
          for(let sT of subTree){
            commands.push("execute "+name+" " + exp.cond.value + " run " + sT)
          }
        }
      } else {
        let subTree = js(exp.then).split("\n")
        for(let sT of subTree){
          commands.push("execute "+name+" " + exp.cond.value + " run " + sT)
        }
      }
