...
function TokenStream(input) {
	...
	// Keywords
	var keywords = " if then else true false for as at asat positioned modal align dimension rotated" + " anchored while do forEach raycast stop continue switch case default var bool" + " boolean tag score const override function run modaljs Map Array ";
	var operators = "+-*/%=&|<>!";
	var puncs = ",;:(){}[]";
	var whitespaces = " \r\n\t";
...
generate = function (exp, oldFile, prjName = "prjname", prjPath = "") {
	...
	function js_binary(exp) {
		...
		else if (exp.operator && vars.indexOf(exp.left.value) >= 0 && exp.right.type === "var" && vars.indexOf(exp.right.value) >= 0)
