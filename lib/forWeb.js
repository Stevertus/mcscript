...
function TokenStream(input) {
	...
	// Keywords
	var keywords =
		" if then else true false for as at asat positioned modal align dimension rotated" +
...
parse = function (input) {
	...
	function parse_if() {
		...
		if (!is_punc("{"))
			input.croak(
				"The statements must be in a {} block. Please add a {."
			);
	...
	function parse_atom() {
		return maybe_call(function () {
			...
			if (is_kw("var") || is_kw("score"))
				return parse_var(input.peek().value);
...
generate = function (exp, oldFile, prjName = "prjname", prjPath = "") {
	...
	function gen_modaljs(modal, exp) {
		...
		let virtualRes = new Function(
			...Object.keys(jsArgs).concat([modal.after[0].value])
		);
