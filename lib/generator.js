...
generate = function (exp, oldFile, prjPath = oldFile.split("/").indexOf("functions"), prjName = "noname") {
	...
	function js_binary(exp) {
		...
		if (exp.operator === "=") {
			return js_assign(exp);
		} else if (exp.operator === "+=" && vars.indexOf(exp.left.value) >= 0 && exp.right.type === "num") {
			return "scoreboard players add " + selector.value + " " + exp.left.value + " " + exp.right.value;
		} else if (exp.operator === "-=" && vars.indexOf(exp.left.value) >= 0 && exp.right.type === "num") {
			return "scoreboard players remove " + selector.value + " " + exp.left.value + " " + exp.right.value;
		} else if (exp.operator && exp.right.type === "var") {
			return "scoreboard players operation " + selector.value + " " + exp.left.value + " " + exp.operator + " " + selector2.value + " " + exp.right.value;
		} else {
			console.warn(exp.left.value + " might be not declared or you messed something up!\n in: " + oldFile);
		}
	}
	...
	function js_assign(exp) {
		...
		if (exp.right.type === "num") {
			return `scoreboard players set ${selector.value} ${exp.left.value} ${exp.right.value}`;
		}
		if (exp.right.type === "command") {
			return `execute store result score ${selector.value} ${exp.left.value} run ${exp.right.value}`;
		} else if (exp.right.type === "var") {
			return `scoreboard players operation ${selector.value} ${exp.left.value} = ${selector2.value} ${exp.right.value}`;
		} else {
			console.warn(`${exp.left.value} is not declared!\n in: ${oldFile}`);
			// throw new Error(`${exp.left.value} is not declared!\n in: ${oldFile}`);
		}
	}
			...
			if (vars.indexOf(exp.name) < 0) {
				...
				if (genScore) {
					if (processed["mcscript/load"]) {
						processed["mcscript/load"] += `scoreboard objectives remove ${exp.name}\n`;
						processed["mcscript/load"] += `scoreboard objectives add ${exp.name} ${type}\n`;
					} else {
						processed["mcscript/load"] = `#extend: ${prjPath}/mcscript/load\n# please do not touch this file!\n# it is used by the compiler!\n`;
						processed["mcscript/load"] += `scoreboard objectives remove ${exp.name}\n`;
						processed["mcscript/load"] += `scoreboard objectives add ${exp.name} ${type}\n`;
					}
				}
				...
				console.error("\x1b[31m", "The variable " + exp.name + " maybe was declared multiple times!", "\x1b[0m");
	...
	function js_assignBool(exp) {
		if (!exp.left.selector && !tagOhne) {
			if (processed["mcscript/load"]) {
				processed["mcscript/load"] += "execute unless entity @e[tag=mcscriptTags] at @p run summon armor_stand ~ ~ ~ {Tags:[mcscriptTags],Invisible:1,Invulnerable:1,NoGravity:1}\n";
			} else {
	...
	function make_switch(exp) {
		...
		for (let cased of exp.cases) {
			...
			cased.cond.push({
				type: "str",
				value: "entity @s[tag=!mcscriptSwitch]",
			});
			res += make_if(cased);
		}
		if (def) {
		...
		for (let cond of exp.cond) {
			...
			if (cond.type === "var" && tags.indexOf(cond.value) !== -1 && (name === "unless" || name === "if")) {
				...
				if (cond.selector && cond.selector.value.slice(-1) === "]") cond.value = cond.selector.value.substr(0, cond.selector.value.length - 1) + ",tag=" + cond.value + "]";
					...
					cond.value = "@e[tag=mcscriptTags,tag=" + cond.value + "]";
				}
			} else if (cond.type === "var" && vars.indexOf(cond.value) !== -1 && (name === "unless" || name === "if")) {
				nameOp += " score";
				if (cond.selector) {
					cond.value = cond.selector.value + "matches 1..";
				} else {
					cond.value = cond.value + " matches 1..";
				}
			} else if (cond.type === "var" && /@[peas]\[.*\]/.test(cond.value) && name === "unless") {
				// nameOp += " entity";
				cond.value = "entity " + cond.value;
			}
			...
			if (cond.op && cond.op === "or") {
				if (cond.value !== undefined) {
					savedExtendedCommands.push("execute" + " " + nameOp + " " + cond.value + nestedMiddle + " run ");
				} else if (cond.type === "binary") {
					savedExtendedCommands.push("execute" + " " + nameOp + " " + testforBinary(cond) + nestedMiddle + " run ");
				}
			} else {
				if (cond.value !== undefined) {
	...
	function gen_modaljs(modal, exp) {
		...
		for (var i = 0; i < modal.args.length; i++) {
			let arg = modal.args[i];

			if (/^\s*\.{3}/.exec(arg.value) !== null) {
				let args = [];
				for (let j = i; j < exp.args.length; j++) {
					args.push(exp.args[j].value);
				}
				let argsName = /[a-zA-Z$_][0-9a-zA-Z$_]*/.exec(arg.value);
				jsArgs[argsName] = args;
				break;
			} else if (arg.type == "var") {
				jsArgs[arg.value] = exp.args[i].value;
			} else if (arg.type == "assign") {
				jsArgs[arg.left.value] = arg.right.value;
				if (exp.args[i]) {
					jsArgs[arg.left.value] = exp.args[i].value;
				}
			}
		}
...
getFiles = function (dir, end = "mcscript", sortByIndex = true) {
	let results = [];
	...
	list.forEach(function (file) {
		...
		if (stat && stat.isDirectory()) {
			...
			results = results.concat(getFiles(file, end, false));
		} else {
			/* Is a file */
	...
	if (sortByIndex) {
		// sort the files by #index: <int | double>
		return results
			.map((fpath) => {
				let idx = 1.0;
				const stat = fs.statSync(fpath);
				if (stat && stat.isFile) {
					let idxDeclaras = fs.readFileSync(fpath, "utf8").match(/(?<=(\n|^)\#index\:\s*)-?\d+(\.\d+)?(?=\s*\n|$)/g);
					if (idxDeclaras) idx = idxDeclaras[0] * 1.0;
				}
				return {
					fpath: fpath,
					index: idx,
...
exports.getModals = function (ast, oldFile, sync = false) {
	// TODO sync
	let data = generate(ast, oldFile);
	let file = oldFile.replace(".mcscript", "");
	ensureDirectoryExistence(file);
	if (sync) {
		fs.writeFileSync(file + ".json", JSON.stringify(modals));
...
exports.getAst = function (ast, oldFile, sync = false) {
	let file = oldFile.replace(".mcscript", "");
	ensureDirectoryExistence(file);
	if (sync) {
		fs.writeFileSync(file + ".json", JSON.stringify(ast));
...
const parseCode = function (ast, oldFile, preOptions = null, sync = false) {
	if (sync) {
		if (preOptions) {
		...
		checkFilename(data, oldFile, function (file, dat) {
			...
			ensureDirectoryExistence(file);
			try {
				fs.writeFileSync(file + ".mcfunction", dat);
				console.log(`\x1b[36mGenerated \x1b[33m${file}.mcfunction\x1b[0m`);
			} catch (err) {
				console.error("Error occured when writing file synchronously.");
				console.error("File: ", file + ".mcfunction");
				console.error(err);
			}
		});
		...
		checkFilename(
			...
			function (file, dat) {
				...
				ensureDirectoryExistence(file);
				try {
					fs.appendFileSync(file + ".mcfunction", dat);
					console.log("\x1b[36m", "Extended ", "\x1b[33m", file + ".mcfunction", "\x1b[0m");
				} catch (err) {
					console.error("Error occured appending file async: ", file + ".mcfunction");
					console.error(err);
					throw err;
				}
			},
...
// only node.js
exports.getVars = function (ast, oldFile, preOptions = null, sync = false) {
	parseCode(ast, oldFile, preOptions, sync);
	return {
		vars: vars,
		modals: modals,
		tags: tags,
...
function checkFilename(data, oldFile, then, ignoreSingle = false) {
	if (data.length > 1) {
		...
		if (!ignoreSingle) {
			then(oldFile.replace(/.mcscript$/, ""), data);
		}
	}
}
