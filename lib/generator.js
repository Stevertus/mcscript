...
generate = function (
	...
	function js_assign(exp) {
		...
		if (vars.indexOf(exp.left.value) >= 0 && exp.right.type === "command")
			...
			throw new Error(
				`${exp.left.value} is not declared!\n in: ${oldFile}`
			);
		}
	}
	...
	function js_assignBool(exp) {
		...
		let selector = exp.left.selector
			...
			: "@e[tag=mcscriptTags]";
		if (exp.right.value)
			return "tag " + selector + " add " + exp.left.value;
	...
	function make_switch(exp) {
		...
		if (def) {
			def.cond = [
				{ type: "str", value: "entity @s[tag=!mcscriptSwitch]" },
			];
			res += make_if(def);
		}
	...
	function gen_run_func(exp) {
		if (exp.func.prog) {
			gen_func(exp.func);
			return (
				"function " + prjName + ":" + exp.func.name.replace("./", "")
			);
		}
		if (exp.func.name)
			return (
		...
		for (let cond of exp.cond) {
			...
			if (
				...
				if (cond.selector && cond.selector.value.slice(-1) === "]")
					cond.value =
						cond.selector.value.substr(
							0,
							cond.selector.value.length - 1
						) +
						",tag=" +
						cond.value +
						"]";
					...
					cond.value =
						cond.selector.value + "[tag=" + cond.value + "]";
			...
			if (exp.nest) {
				for (let nested of exp.nest) {
					for (let nestedCond of nested.cond) {
						...
						let additions = "";
						if (nestedCond.not && nested.type === "if")
							keyWord = "unless";
						if (nestedCond.not && nested.type === "unless")
							keyWord = "if";
	...
	function gen_raycast(exp) {
		...
		whileObj.then.prog.push({
			type: "command",
			value:
				"scoreboard players add raycast" + id + " mcscript_raycast 1",
		});
		let end = {
			type: "if",
			cond: [
		...
		if (exp.entity) {
			...
			if (entity.slice(-1) === "]")
				entity =
					entity.substr(0, entity.length - 1) +
					",distance=..0.7,sort=nearest]";
...
const parseCode = function (ast, oldFile, preOptions = null, sync = false) {
	if (sync) {
		if (preOptions) {
			vars = vars.concat(
				preOptions.vars.filter((x) => vars.indexOf(x) === -1)
			);
			...
			tags = tags.concat(
				preOptions.tags.filter((x) => tags.indexOf(x) === -1)
			);
		...
		if (
			oldFile.split("/").splice(-2).join("/") ===
				"functions/load.mcscript" ||
		...
		checkFilename(data, oldFile, function (file, dat) {
			...
			try {
				...
				console.log(
					`\x1b[36mGenerated \x1b[33m${file}.mcfunction\x1b[0m`
				);
			} catch (err) {
		...
		checkFilename(
			...
			function (file, dat) {
				...
				try {
					...
					console.error(
						"Error occured appending file async: ",
						file + ".mcfunction"
					);
					console.error(err);
					throw err;
