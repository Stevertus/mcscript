    ...
    con.forEach(x => {
      ...
      if(x.substr(0, 5) == '.repl'){
        ...
        if(strArr[1][0] == '/'){
          ...
          tempVal = cnst.value.replace(regs, strArr[3])
        } else tempVal = tempVal.split(strArr[1]).join(strArr[3])
        con[con.indexOf(x)] = tempVal + strArr[4].substr(1)
      } else {
        if(con.indexOf(x) != 0) con[con.indexOf(x)] = tempVal + con[con.indexOf(x)]
      }
    })
    res = con.join('');
  }
