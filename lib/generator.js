...
generate = function (
	...
	function make_indent(exp, name) {
		...
		for (let cond of exp.cond) {
			...
			if (cond.type === "var" && (name === "unless" || name === "if")) {
				if (tags.indexOf(cond.value) !== -1) {
					nameOp += " entity";
					if (
						cond.selector &&
						cond.selector.value.slice(-1) === "]"
					) {
						cond.value =
						...
						cond.value =
							"@e[tag=mcscriptTags,tag=" + cond.value + "]";
					}
				} else if (vars.indexOf(cond.value) !== -1) {
					nameOp += " score";
					if (cond.selector) {
						cond.value = cond.selector.value + "matches 1..";
					} else {
						cond.value += " matches 1..";
					}
				} else {
					nameOp += ` score ${
						cond.selector
							? cond.selector.value
							: "@e[tag=mcscriptTag,limit=1]"
					}`;
					cond.value += " matches 1..";
				}
			}
