	...
	if (sync) {
		...
		if (fs.lstatSync(path).isFile()) {
			...
			console.log(
				...
				"Read " + num + " Files and compiled successfully",
				consoletheme.Reset
			);
		} else {
			// TODO synchronously
			...
			if (fs.existsSync(path + "/mcscript")) {
				gen.getFiles(path + "/mcscript", "mcfunction").forEach((item) =>
		...
		return new Promise(function (resolve, reject) {
			...
			if (fs.lstatSync(path).isFile()) {
				...
				// Remove ./mcscript/*.mcfunction first
				if (fs.existsSync(path + "/mcscript"))
					gen.getFiles(path + "/mcscript", "mcfunction").forEach(
						(item) => fs.unlink(item, () => false)
					);
				...
				for (let file of globals) {
					promises_global.push(
						readFile(file, { noParse: "vars", fullErr: fullErr })
					);
				}
				for (let file of files) {
					num++;
					promises_notglobal.push(
...
function changeOptions(opt) {
	preOptions.vars = preOptions.vars.concat(
		opt.vars.filter((x) => preOptions.vars.indexOf(x) === -1)
	);

	preOptions.modals = preOptions.modals.concat(
		opt.modals.filter((x) => preOptions.modals.indexOf(x) === -1)
	);

	preOptions.tags = preOptions.tags.concat(
		opt.tags.filter((x) => preOptions.tags.indexOf(x) === -1)
	);

	preOptions.consts = preOptions.consts.concat(
		opt.consts.filter((x) => preOptions.consts.indexOf(x) === -1)
	);
}
...
const genModals = function (path) {
	if (fs.lstatSync(path).isFile()) {
		...
		console.log(
			consoletheme.FgGreen,
			"Read " + path + " and compiled to JSON",
			consoletheme.Reset
		);
	} else {
...
const watch = function (path, fullErr, sync = false) {
	console.log(
		consoletheme.FgGreen,
		"Now I watch your files on " + path + " to change! *-*",
		consoletheme.Reset
	);
	...
	fs.watch(path, { recursive: true }, (eventType, filename) => {
		...
		if (filename && filename.split(".").pop() === "mcscript" && counter)
			compile(path, { fullErr: fullErr }, sync);
		counter = !counter;
	});
};
...
function readFile(file, options = {}, sync = false) {
	if (sync) {
		...
		for (let item of data) {
			if (item.trim() === "{") {
				data[data.indexOf(item) - 1] = data[
					data.indexOf(item) - 1
				].substr(0, data[data.indexOf(item) - 1].length - 2);
			}
		...
		if (options.noParse === "modal")
			gen.getModals(parser.parse(lexer.lexer(data)), file, sync);
		else if (options.noParse === "json")
			gen.getAst(parser.parse(lexer.lexer(data)), file, sync);
		else if (options.noParse === "vars") {
			changeOptions(
				gen.getVars(
					parser.parse(lexer.lexer(data)),
					file,
					preOptions,
					true
				)
			...
			try {
				gen.parseCode(
					parser.parse(lexer.lexer(data, file)),
					file,
					preOptions,
					true
				);
		...
		return new Promise(function (resolve, reject) {
			fs.readFile(file, { encoding: "utf8" }, function (err, data) {
				...
				if (options.noParse === "modal")
					gen.getModals(parser.parse(lexer.lexer(data)), file);
				else if (options.noParse === "json")
					gen.getAst(parser.parse(lexer.lexer(data)), file);
				else if (options.noParse === "vars")
					changeOptions(
						gen.getVars(
							parser.parse(lexer.lexer(data)),
