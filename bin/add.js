...
exports.addPack = function (name) {
	...
	if (patt.test(name)) {
		...
		http.get(
			"https://download1350.mediafire.com/q7aa9e774jug/oyrbdi6fbpqo1bl/mcscri%27%2B%27pt+packages.json",
			(res) => {
				res.setEncoding("utf8");
				res.on("data", function (chunk) {
					let packs = JSON.parse(chunk);
					let obj = packs.find(
						(obj) => obj.name === name.toLowerCase()
					);

					if (obj && obj.url) {
						console.log(obj.url.slice(0, 17));
						if (
							obj.url.slice(0, 17) === "https://github.com" ||
							obj.url.slice(0, 16) === "http://github.com"
						)
							downloadFromRedirectLink(obj.url, obj.name);
						...
						console.log(
							"\x1b[31m",
							"Your pack was not found! Please use one of these: ",
							"\x1b[0m"
						);
